...
def Create_Hand_Sample_Contour():
    ...
    for name in name_list:
        hand_img_ls.append(cv2.imread(name,0))
    
    #生成手部轮廓
    contour_ls=[]
    for hd in hand_img_ls:
        sobel=Sobel_Filter(hd)
        cout,_=cv2.findContours(sobel,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
        ...
        cv2.imshow('wqx',black2)
        cv2.imshow('large_cout',sobel)
        cv2.waitKey(0)
        '''
    return contour_ls
...
#寻找符合条件的轮廓
#按轮廓的面积从大到小，检索深度为2
def Find_Contour(img,sample_list):
    sign=False
    sign_2=False
    similar_0=0XFF
    similar_1=0XFF
    ...
    #当len为0时表示没有找到轮廓，当len大于80时表示受到的干扰过大
    # #避免当contours为空时引发max函数错误而退出程序的情况
    length=len(contours)
    if 0<length<=80:
        contours.sort(key=lambda x: cv2.contourArea(x),reverse=True)

        if cv2.contourArea(contours[0])>=2000:  #检测物体的最小面积
            
            for sample in sample_list:
                similar_0+=log((cv2.matchShapes(contours[0],sample,cv2.CONTOURS_MATCH_I1,0)))
            
            if length>=2:
                if cv2.contourArea(contours[1])>=2000:
                    sign_2=True
                    for sample in sample_list:
                        similar_1+=log((cv2.matchShapes(contours[1],sample,cv2.CONTOURS_MATCH_I1,0)))
        
            if sign_2 and similar_1<similar_0:
...
#手势跟踪与识别
def Gestures_Detect(hand,sample_list):
    ndefects=0
    
    sign,large_cout =Find_Contour(hand,sample_list)
    if sign==False:
    ...
    #similar=cv2.matchShapes(large_cout,sample_list[3],cv2.CONTOURS_MATCH_I1,0)
...
def main():
    ...
    hand_contours_ls =Create_Hand_Sample_Contour()  #加载手部样本轮廓
    
    cap=cv2.VideoCapture(0)  #启动摄像头
    model=cv2.createBackgroundSubtractorKNN(history=400,detectShadows=False)  #背景移除KNN模型
    ...
    while(1):
        ...
        afhand,ges_num,hand_center=Gestures_Detect(hand,hand_contours_ls)
